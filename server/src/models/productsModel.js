    ...
    try {
        ...
        if (category_id > 0) {
            formattedQuery = format(
                'SELECT p.product_id, p.name, p.category_id, p.description, p.price, p.stock, p.url_image, p.isactive, COALESCE(CEIL(AVG(r.stars)),0) as stars FROM products p LEFT JOIN reviews r ON p.product_id = r.product_id GROUP BY p.product_id HAVING p.category_id = %s ORDER BY %s %s LIMIT %s OFFSET %s',
...
export const updateProductModel = async (id, { name, category_id, description, price, stock, url_image, isactive }) => {
    const consultaSQL = {
        text: 'UPDATE products SET name=$1, category_id=$2, description=$3, price=$4, stock=$5, url_image=$6, isactive=$7 WHERE product_id=$8 RETURNING *',
